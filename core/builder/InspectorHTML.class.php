				...
				if (! $valido) {
					if (! $corregir) {
						if ($showError) {
							return array (
									"errorType" => "minSize",
									"errorMessage" => "La longitud es menor a " . $parametros ['minSize'] 
							);
						}
						return false;
					}
			...
			if (isset ( $parametros ['min'] )) {
				...
				if (! $valido) {
					if ($showError) {
						return array (
								"errorType" => "min",
								"errorMessage" => "El número es menor a " . $parametros ['min'] 
						);
					}
					return false;
				}
				...
				if (! $valido) {
					if (! $corregir) {
						if ($showError) {
							return array (
									"errorType" => "maxSize",
									"errorMessage" => "La longitud es mayor a " . $parametros ['maxSize'] 
							);
						}
						return false;
					}
			...
			if (isset ( $parametros ['max'] )) {
				...
				if (! $valido) {
					if ($showError) {
						return array (
								"errorType" => "max",
								"errorMessage" => "El número es mayor a " . $parametros ['max'] 
						);
					}
					return false;
				}
			...
			if (isset ( $parametros ['custom'] )) {
				...
				if (! $valido) {
					if ($showError) {
						return array (
								"errorType" => "custom",
								"errorMessage" => "El campo no es del tipo " . $parametros ['custom'] 
						);
					}
					return false;
				}
