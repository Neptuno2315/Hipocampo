...
if (isset($_REQUEST['funcion'])) {
    ...
    switch ($_REQUEST['funcion']) {
        ...
        case 'consultarZonasEstudio':
            ...
            $cadenaSql = $this->sql->getCadenaSql('consultar_datos_usuario', $_REQUEST['usuario']);

            $usuario = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda")[0];
            ...
            foreach ($resultado as $valor) {
                ...
                $cadenaACodificar .= "&id_zona=" . $valor['id_zona_estudio'];
                $cadenaACodificar .= "&titulo_proyecto=" . $valor['titulo_proy'];
                $cadenaACodificar .= "&firma_digital=TRUE";
                // Codificar las variables
                $enlace = $this->miConfigurador->getVariableConfiguracion("enlace");
                $cadena = $this->miConfigurador->fabricaConexiones->crypto->codificar_url($cadenaACodificar, $enlace);
                ...
                // URL definitiva
                $urlDocumento = $url . $cadena;
                if ($usuario['firma_usuario'] != null) {

                    $documento_firma = (is_null($valor['recomendaciones']) == true) ? "" : "<center><a href='" . $urlDocumento . "'><img src='" . $UrlDirectorioIconos . "descargar.png 'width='20px'></a></center>";
