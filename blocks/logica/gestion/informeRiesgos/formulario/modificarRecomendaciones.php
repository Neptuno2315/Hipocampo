	...
	function miForm() {
		...
		{ /*
		   ...
		   */
			...
			$cadenaSql = $this->miSql->getCadenaSql ( "consultar_recomendacion_particular", $_REQUEST ['id_recomendacion'] );
			$recomendacion = $esteRecursoDB->ejecutarAcceso ( $cadenaSql, "busqueda" );
			$recomendacion = $recomendacion [0];
			$arrayRecomendacion = array (
					"riesgo" => $recomendacion ['riesgo'],
					"acciones" => $recomendacion ['acciones_prv'],
					"senalizacion" => $recomendacion ['senalizacion_ext'] 
			);
			
			$_REQUEST = array_merge ( $_REQUEST, $arrayRecomendacion );
		}
